if test $# -eq 0 #comprobamos la cantidad de argumentos que hay, la variable $# devuelve la cantidad de argumentos (a parte del nombre del script), y el -eq compara lo que hay antes con lo que hay después
...
elif test $# -eq 1 #si el numero de argumentos es 1. Como solo hay un argumento, este deberá ser la columna por la cuál queremos ordenar la lista de passwd. De no ser así, se mostrará un error.
then
	#echo "Hay un argumento"
	case $1 in #utilizamos el comando case para seleccionar la opción en función del primer comando
		login)
			sort -k 1 /etc/passwd #la opción de sort -k permite ordenar el archivo en función de la columna indicada después. En este caso, ordenamos por la primera columna ya que el parámetro pasado es login
...
elif test $# -eq 2 #si el numero de argumentos es 2. Si hay dos argumentos, la sintaxis debe ser [-a|-z] [login|uid|gid]. De no ser así y si los argumentos están cambiados de orden, se dará un error. Este caso se reduce a revisar si el primer argumentos es -a o -z y en otro caso, salir con error 1.
then
	...
	case $1 in #primer case, correspondiente a la opción del script
		...
		-z)
			case $2 in
				login)
					sort -r -k 1 /etc/passwd #nótese que en este sort utilizamos la opción -r, para ordenar de forma inversa, puesto que la opción elegida fue -z
					;;
				uid) 
